...
export const displayMeetingTime = (timeframe) => {
  const startTime = get12HourTime(timeframe.start_time).toLowerCase();
  return startTime.slice(0, -3); // Displaying only the start time without the minutes and AM/PM
};
...
export const getYearDaysFromTodayDateTimestamp = () => {
  const date = new Date();
