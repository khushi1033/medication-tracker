...
app.post('/nylas/exchange-mailbox-token', express.json(), async (req, res) => {
  ...
  const userM = await mongoDb.createOrUpdateUser(user.id, emailAddress, {
    accessToken,
    emailAddress,
  });
...
app.get('/nylas/get-prem', isAuthenticated, (req, res) =>
  route.getUserPremiumStatus(req, res)
);
...
// Add route for getting 20 latest calendar events
app.get('/nylas/read-med/:title', isAuthenticated, (req, res) =>
  route.readMedication(req, res)
);
...
// Add route for upgrading user to premium
app.put('/nylas/upgrade-user', isAuthenticated, (req, res) =>
  route.upgradeUser(req, res)
);


app.put('/nylas/downgrade-user', isAuthenticated, (req, res) =>
  route.downgradeUser(req, res)
);
